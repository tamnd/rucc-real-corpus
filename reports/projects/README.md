# Every project

[Back to the report](README.md).

One row per project and one page behind each row. `cells` counts every level this run covered, so a project that passed at every one of them reads 4 of 4.

The `files` and `lines` columns are the size of the pinned source, counted before anything is built, and they are here so that the rest of the row can be read. A project that fails at one level out of six is a different piece of news at three hundred lines than at thirty thousand.

| project | rung | files | lines | cells passed | behind gcc on tests |
| --- | --- | ---: | ---: | ---: | ---: |
| [blake2](blake2.md) | R1 | 62 | 65,842 | 4 of 4 | none |
| [brotli](brotli.md) | R2 | 109 | 42,932 | 4 of 4 | none |
| [bzip2](bzip2.md) | R1 | 15 | 8,127 | 4 of 4 | none |
| [c4](c4.md) | R0 | 2 | 535 | 4 of 4 | none |
| [cjson](cjson.md) | R2 | 99 | 22,367 | 4 of 4 | none |
| [cmocka](cmocka.md) | R2 | 51 | 9,613 | 4 of 4 | none |
| [coremark](coremark.md) | R0 | 16 | 4,543 | 4 of 4 | none |
| [gdbm](gdbm.md) | R2 | 100 | 28,049 | 4 of 4 | none |
| [heatshrink](heatshrink.md) | R0 | 11 | 4,458 | 3 of 4 | none |
| [incbin](incbin.md) | R0 | 3 | 806 | 1 of 4 | none |
| [jsmn](jsmn.md) | R0 | 6 | 1,168 | 4 of 4 | none |
| [jtckdint](jtckdint.md) | R0 | 3 | 854 | 4 of 4 | none |
| [libcheck](libcheck.md) | R2 | 74 | 18,978 | 4 of 4 | none |
| [libconfig](libconfig.md) | R2 | 42 | 15,493 | 4 of 4 | none |
| [libexpat](libexpat.md) | R2 | 80 | 37,804 | 4 of 4 | none |
| [libgmp](libgmp.md) | R2 | 1,054 | 212,797 | 4 of 4 | none |
| [libjansson](libjansson.md) | R2 | 49 | 18,654 | 4 of 4 | none |
| [libjpeg](libjpeg.md) | R2 | 84 | 51,338 | 4 of 4 | none |
| [libmpfr](libmpfr.md) | R2 | 507 | 147,331 | 0 of 4 | 8 |
| [libpng](libpng.md) | R2 | 100 | 91,353 | 4 of 4 | none |
| [libpsl](libpsl.md) | R2 | 14 | 4,236 | 4 of 4 | none |
| [libsir](libsir.md) | R1 | 73 | 19,927 | 4 of 4 | none |
| [libsodium](libsodium.md) | R2 | 354 | 60,896 | 4 of 4 | none |
| [libtommath](libtommath.md) | R2 | 177 | 30,479 | 4 of 4 | none |
| [libuv](libuv.md) | R2 | 360 | 109,292 | 4 of 4 | none |
| [libyaml](libyaml.md) | R2 | 23 | 16,979 | 4 of 4 | none |
| [linenoise](linenoise.md) | R1 | 4 | 4,127 | 4 of 4 | none |
| [llama2.c](llama2.c.md) | R0 | 5 | 2,398 | 4 of 4 | none |
| [lmdb](lmdb.md) | R1 | 26 | 19,799 | 4 of 4 | none |
| [lz4](lz4.md) | R1 | 69 | 28,036 | 4 of 4 | none |
| [minunit](minunit.md) | R1 | 2 | 477 | 4 of 4 | none |
| [monocypher](monocypher.md) | R1 | 11 | 23,116 | 4 of 4 | none |
| [ncompress](ncompress.md) | R1 | 2 | 1,785 | 4 of 4 | none |
| [oniguruma](oniguruma.md) | R2 | 89 | 102,240 | 4 of 4 | none |
| [parson](parson.md) | R0 | 3 | 3,672 | 4 of 4 | none |
| [pcre2](pcre2.md) | R2 | 85 | 149,512 | 4 of 4 | none |
| [picohttpparser](picohttpparser.md) | R0 | 6 | 1,538 | 4 of 4 | none |
| [rpmalloc](rpmalloc.md) | R1 | 10 | 6,823 | 4 of 4 | none |
| [sds](sds.md) | R0 | 4 | 1,701 | 4 of 4 | none |
| [tinf](tinf.md) | R0 | 10 | 4,002 | 4 of 4 | none |
| [tinycthread](tinycthread.md) | R1 | 4 | 1,363 | 4 of 4 | none |
| [tinyexpr](tinyexpr.md) | R0 | 9 | 2,496 | 4 of 4 | none |
| [uzlib](uzlib.md) | R1 | 14 | 1,954 | 4 of 4 | none |
| [xxhash](xxhash.md) | R1 | 44 | 63,655 | 4 of 4 | none |
| [zlib](zlib.md) | R1 | 75 | 42,769 | 4 of 4 | none |
| [zstd](zstd.md) | R2 | 277 | 137,191 | 4 of 4 | none |
