...
/// One demand the amalgamation makes, and the smallest project below it that makes the same one.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Row {
    /// The tag, as `features.toml` spells it.
    pub tag: String,
    /// How many places in `sqlite3.c` use it.
    pub sites: u32,
    /// What was counted, so the number can be argued with.
    pub evidence: String,
    /// The smallest project on the list that demands the same thing, if there is one.
    pub smallest: Option<Reacher>,
}

impl Row {
    /// Whether this demand is part of the residue: something SQLite needs and nothing below it
    /// exercises, so the ladder will not have found out about it before the climb.
    #[must_use]
...
/// The SQLite column of `spec/10-feature-demand.md` section 10.7.
///
/// The milestone's question in one table. Every demand the amalgamation makes, the smallest project
/// below it that makes the same demand, and what the run currently says about that project. The
/// residue is the count that decides RC2: if it is small the ladder worked, because almost
/// everything SQLite needs was reachable from something cheaper, and if it is large then the small
/// projects were never going to find out what the big one needs and the ladder is the wrong shape.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Column {
    /// One row per demand, residue first and then by how much of SQLite is behind it.
    pub rows: Vec<Row>,
    /// The tags that were looked for and are not in the amalgamation, with what was looked for.
    pub absent: Vec<(String, String)>,
    /// The amalgamation the counts came from.
    pub version: String,
...
impl Column {
    /// Match every measured demand against the map.
    #[must_use]
    pub fn of(map: &Map, sqlite: &Sqlite) -> Self {
        let mut rows: Vec<Row> = sqlite
            .demands()
            .map(|measured| Row {
                tag: measured.tag.clone(),
                sites: measured.sites,
                evidence: measured.evidence.clone(),
                smallest: map.find(&measured.tag).and_then(Demand::smallest).cloned(),
            })
            .collect();
        // Residue first, because it is the answer to the milestone's question and burying it under
        // twenty rows that are fine would be a way of not answering. Then by site count, which is
        // roughly how much of SQLite stops working if the demand is not met, and the tag last so
        // that the committed file does not shuffle itself between runs.
        rows.sort_by(|a, b| {
            b.residue()
                .cmp(&a.residue())
        ...
        let mut absent: Vec<(String, String)> = sqlite
            .absent()
            .map(|measured| (measured.tag.clone(), measured.evidence.clone()))
            .collect();
        absent.sort();
        Self {
            rows,
            absent,
            version: sqlite.source.version.clone(),
            lines: sqlite.source.lines,
        }
    }

    /// How many demands have nothing below them.
    #[must_use]
    pub fn residue(&self) -> usize {
        self.rows.iter().filter(|row| row.residue()).count()
    }

    /// Whether there is anything to print.
    #[must_use]
    pub fn is_empty(&self) -> bool {
        self.rows.is_empty() && self.absent.is_empty()
    }
}
...
/// people follow it.
#[must_use]
pub fn render(map: &Map, column: &Column, provenance: &str) -> String {
...
/// The SQLite column, from `spec/10-feature-demand.md` section 10.7.
///
/// Written into the same file as the map on purpose. The two are one question asked twice: the map
/// says what to implement next, and the column says whether doing that ever gets us to SQLite. Kept
/// in separate files they would go stale against each other, and the one that went stale would be
/// the one people quote.
fn sqlite_column(out: &mut String, column: &Column) {
    ...
    let _ = writeln!(
        out,
        "This is the number RC2 turns on. The residue is the demands with nothing below them, which are the things the ladder was never going to find out about before the climb. It is currently {} of {}.\n",
        column.residue(),
        column.rows.len()
    );

    out.push_str("| demand | sites | smallest reacher below | rung | outcome |\n");
    out.push_str("|---|---|---|---|---|\n");
    for row in &column.rows {
        let (project, rung, outcome) = row.smallest.as_ref().map_or_else(
            || ("nothing below it".to_string(), String::new(), String::new()),
            |reacher| {
                (
                    format!("`{}`", reacher.project),
                    format!("R{}", reacher.rung.as_u8()),
                    reacher
                        .outcome
                        .map_or_else(|| "not measured".to_string(), |o| o.name().to_string()),
                )
        ...
        let _ = writeln!(
            out,
            "| `{}` | {} | {} | {} | {} |",
            row.tag, row.sites, project, rung, outcome
        );
    }
    out.push('\n');

    let residue: Vec<&Row> = column.rows.iter().filter(|row| row.residue()).collect();
    if !residue.is_empty() {
        out.push_str("### The residue\n\n");
        out.push_str("Each of these is a project worth admitting, or an argument that the demand is not worth covering below SQLite. Both are decisions, and they are the RC2 decisions.\n\n");
        for row in residue {
            let sites = if row.sites == 1 {
                "one site".to_string()
            } else {
                format!("{} sites", row.sites)
            };
            let _ = writeln!(out, "- `{}`, {sites}: {}", row.tag, row.evidence);
        }
        out.push('\n');
    }

    if !column.absent.is_empty() {
        out.push_str("### Looked for and not there\n\n");
        out.push_str("These rows are why the residue above can be believed. A list that only records what it found cannot be told apart from a list nobody finished, so what was searched for and missing is written down too.\n\n");
        for (tag, evidence) in &column.absent {
            let _ = writeln!(out, "- `{tag}`: {evidence}");
        }
        out.push('\n');
    }
}
...
"#;
    ...
    fn a_map_with_no_run_behind_it_says_so_rather_than_printing_an_empty_column() {
        ...
        let text = render(&map, &Column::of(&map, &Sqlite::default()), "from nothing");
        assert!(text.contains("No run records were read"));
        assert!(text.contains("not measured"));
        assert!(
            !text.contains("## The SQLite column"),
            "a corpus with no measurements has no column, rather than an empty heading"
        );
    }
    ...
    fn the_same_corpus_renders_the_same_way_twice() {
        ...
        let first = render(&map, &Column::of(&map, &sqlite()), "x");
        let again = Map::of(&manifests, &features(), &Exclusions::default(), &[]);
        let second = render(&again, &Column::of(&again, &sqlite()), "x");
        assert_eq!(
            first, second,
...
tag = "bit-builtins"
sites = 10
evidence = "__builtin_clzll and the byte swaps"
...
"#;
    ...
    fn sqlite() -> Sqlite {
        toml::from_str(SQLITE).unwrap()
    }

    #[test]
    fn a_demand_with_something_below_it_names_the_smallest_one() {
        // The point of the column. Two projects demand the same thing SQLite does, and the one
        // worth pointing at is the cheaper of them, because that is where the demand gets tested
        // long before anybody builds an amalgamation.
        let map = Map::of(
            &[
                project("big", 3, &["bit-builtins"]),
                project("small", 0, &["bit-builtins"]),
            ],
            &features(),
            &Exclusions::default(),
            &[],
        );
        let column = Column::of(&map, &sqlite());
        assert_eq!(column.rows.len(), 1);
        assert_eq!(column.rows[0].smallest.as_ref().unwrap().project, "small");
        assert_eq!(column.residue(), 0);
    }

    #[test]
    fn a_demand_with_nothing_below_it_is_the_residue() {
        // The number RC2 turns on, so it has to come out of the data rather than out of a
        // paragraph somebody wrote. Nothing in this corpus demands bit-builtins, so climbing the
        // whole ladder would never have told us anything about it.
        let map = Map::of(
            &[project("a", 0, &["computed-goto"])],
            &features(),
            &Exclusions::default(),
            &[],
        );
        let column = Column::of(&map, &sqlite());
        assert_eq!(column.residue(), 1);
        assert!(column.rows[0].residue());
        let text = render(&map, &column, "x");
        assert!(text.contains("currently 1 of 1"));
        assert!(text.contains("nothing below it"));
    }

    #[test]
    fn a_tag_the_amalgamation_does_not_use_is_never_part_of_the_residue() {
        // A zero row is evidence that somebody looked, not a demand. Counting it as one would
        // inflate the residue with things SQLite does not need and argue the ladder into being
        // cut for no reason.
        let map = Map::of(&[], &features(), &Exclusions::default(), &[]);
        let column = Column::of(&map, &sqlite());
        assert_eq!(column.rows.len(), 1, "only the row with sites in it");
        assert_eq!(column.absent.len(), 1);
        let text = render(&map, &column, "x");
        assert!(text.contains("Looked for and not there"));
        assert!(text.contains("no labels as values anywhere"));
    }
}
