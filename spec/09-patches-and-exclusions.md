...
**An `upstream:` entry is asked of the reference compiler, because that is who it is about.** Condition 2 reads the outcome of the excluded cell, and for an ordinary entry that is our outcome, since the entry says we fail it. An `upstream:` entry says the opposite: the failure is gcc's, and the cell is not evidence about us either way. Asking our half about one of those gets the question backwards, because the day we become able to pass a cell gcc still fails is the day the entry comes true, and reporting it as stale then would push the entry towards deletion at the exact moment it is doing its job. So for an `upstream:` entry condition 2 reads the reference half of the same cell, and it fires when gcc stops failing. A run measured with `--no-baseline` has no reference half to read and says nothing about those entries rather than guessing, which loses a check rather than inventing a finding. Condition 4 is unaffected and still reads our half, since who is talking is a question about the diagnostic we printed.
