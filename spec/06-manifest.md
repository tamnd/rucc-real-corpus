...
**Arbitrary shell.** There is no `pre-build-script` field and there will not be one, because the first one added is the last day the corpus is declarative. A project that needs a step the schema cannot express is either a schema gap, in which case the schema grows a named field with a lint and a document section, or a patch, in which case document 09 governs it.
