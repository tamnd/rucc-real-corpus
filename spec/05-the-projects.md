...
| `git` | `2.55.0` | GPL-2.0 | A5 | D3/E1 | thirty two thousand assertions, `container_of`, `mmap`, and a build with generated headers |
...
**`git` is the largest row on the list and the only one at A5, and it is admitted for the opposite reason to `bash`.** `bash` is here because it stops early and stops on something new. `git` is here because it very nearly does not stop at all: rucc 0.10.2 compiles 548 of its objects and fails in exactly eight files, across exactly two features, both of which were on the list before git arrived. Three of the errors are `xalloca` at tree-diff.c:39, which is the missing `alloca` that `bash` found reached through a different spelling. The other nine are across the seven files that open a socket, where glibc declares `connect`, `accept` and `bind` with a transparent union and rucc drops the attribute, so an ordinary call with a `struct sockaddr *` becomes a type error. Neither is new, and that is what makes the row worth its price: a project that gets that far measures whether a feature landed properly rather than whether it exists, against thirty two thousand assertions instead of against a reduced case.
