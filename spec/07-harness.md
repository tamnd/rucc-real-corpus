...
rrc run --failing runs/latest           only the cells an earlier run did not pass
...
**`rrc run --failing <run>` keeps only the cells that run did not pass.** The cache above is the right answer for a run that wants a whole report out the other end, and it is the wrong answer for somebody who has just changed one thing in the compiler, because the key has the compiler's bytes in it and a new compiler therefore misses on every cell. So the cache makes an unchanged corpus free and this makes a changed compiler cheap, and the two are not the same mechanism. It names a run's output directory or its `records.jsonl` and keeps the cells whose outcome was not a pass, which is wider than the failures on purpose: a cell that was skipped for a missing requirement or was not compared for want of counts is a cell that did not answer, and somebody asking for the work that is left wants those back too. It filters what was already selected rather than adding to it, so `--rung 0 --failing runs/latest` is rung zero's failing cells and not everything's, and a project whose every level it removes drops out of the run instead of appearing as a row with nothing in it. What a run like this writes is a worklist and not a result for the corpus. The number that says whether a rung is green comes from a run that asked for the whole rung, which is what the nightly does and what document 04.0's simultaneity rule requires.
...
**The exception has an exception, and busybox is what found it.** `build.host-cc` is delivered by putting the host compiler in the environment under `CC_FOR_BUILD`, `HOSTCC` and `BUILD_CC`, which works for a build system that reads one of those and does not work for kbuild, whose Makefile says `HOSTCC = gcc` with a plain assignment. That is section 7.8's problem arriving through a different variable, and it fails in the worse direction: make prefers its own assignment to the environment, `gcc` is resolved through the shim, and so the host tool is built by the compiler under test on exactly the projects that generate the most source. The harness therefore says `HOSTCC=<the host compiler>` on the make command line as well, where nothing in a Makefile can overrule it, and says nothing at all when there is no host compiler to name. It is the same mechanism section 7.8 uses for the level and it is here for the same reason.
