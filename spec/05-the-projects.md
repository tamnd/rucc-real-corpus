...
**One case in the suite is a race and the manifest says so rather than pretending otherwise.** `cmdline/repl_paste.py` is driven through a pseudo terminal by `run-tests.py`, which writes a line and then reads whatever has arrived within twenty milliseconds. On a machine that is busy the prompt for the next line lands in the following read instead, and the recorded output no longer matches. GCC 16.2.0 on the reference machine fails it at O0, O1 and O2 and passes it at O3 and Os, in one run, over one source tree, which is as clear a demonstration as this corpus is likely to get that the number has nothing to do with the compiler. So the row carries a `baseline-total` of 998 against a `baseline-tests` of 997, which is the mechanism document 06.2 introduced for GMP: all 998 cases have to be performed, only that one is allowed to be missing, and a second failure or a suite that quietly shrinks is still red. The other 56 cases the port skips are outside both numbers.
