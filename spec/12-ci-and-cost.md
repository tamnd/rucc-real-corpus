...
**A cell takes as many slots as its project asks for, which is one unless the manifest says otherwise.** The width of a run is `--jobs` and it is handed out rather than divided evenly, because one kind of suite scales itself to the machine and one slot is not what it needs. rpmalloc is a thread stress test for an allocator and starts a thread per core deliberately, since contention is the thing under test, so four of its cells at one slot each on an eight core host took the load average to eighty and all four hit the cell limit that one at a time they finish well inside. `limits.cores` in document 06.2 is how a project says so, the scheduler runs the narrow cells first so that a run cannot block every worker behind a queue of wide ones, and a cell that was given the whole width writes `concurrency` of one because nothing was in fact running beside it.
